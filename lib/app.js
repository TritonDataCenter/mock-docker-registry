...
App.prototype.init = function init(cb) {
    ...
    if (self.config.tls) {
        serverOpts.key = fs.readFileSync(self.config.tlsKeyPath);
        serverOpts.cert = fs.readFileSync(self.config.tlsCertPath);
    }
    server = this.server = restify.createServer(serverOpts);
